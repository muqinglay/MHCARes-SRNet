...
class Block_M(nn.Module):
    def __init__(self, nf,
        ...
        self.b1 = arch_util.EResidualBlockM(nf, nf, group=group)
        self.c1 = arch_util.BasicBlock(nf * 2, nf, 1, 1, 0)
        self.c2 = arch_util.BasicBlock(nf * 3, nf, 1, 1, 0)
        self.c3 = arch_util.BasicBlock(nf * 4, nf, 1, 1, 0)
...
class CARN_MHCA(nn.Module):
    def __init__(self, in_nc, out_nc, nf, scale=4, multi_scale=False, group=4):
        super(CARN_MHCA, self).__init__()
        self.scale = scale
        rgb_range = 1
        rgb_mean = (0.4488, 0.4371, 0.4040)
        rgb_std = (1.0, 1.0, 1.0)
        ...
        self.b1 = Block_M(nf, group=group)
        self.b2 = Block_M(nf, group=group)
        self.b3 = Block_M(nf, group=group)
        self.c1 = arch_util.BasicBlock(nf * 2, nf, 1, 1, 0)
        self.c2 = arch_util.BasicBlock(nf * 3, nf, 1, 1, 0)
        self.c3 = arch_util.BasicBlock(nf * 4, nf, 1, 1, 0)
    ...
    def forward(self, x):
        ...
        self.b1 = Blocke(nf, nf)
        self.b2 = Blocke(nf, nf)
        self.b3 = Blocke(nf, nf)
        self.c1 = ops.BasicBlock(nf * 2, nf, 1, 1, 0)
        self.c2 = ops.BasicBlock(nf * 3, nf, 1, 1, 0)
        self.c3 = ops.BasicBlock(nf * 4, nf, 1, 1, 0)

        self.upsample = ops.UpsampleBlock(nf, scale=scale,
                                          multi_scale=multi_scale,
                                          group=group)
        self.exit = nn.Conv2d(nf, out_nc, 3, 1, 1)
